...
int main(int argc, char *argv[]){
	...
	for (int i = 0; i < numTraj; i++){
		...
		dcdCoord *coord = (dcdCoord *)malloc(sizeof(dcdCoord) * numAtom);
		dcdReader(coord, box);
...
void initReader(){
	...
	init = (initInfo *)malloc(sizeof(initInfo) * numAtom);
	...
	qsort(init, numAtom, sizeof(initInfo), compare); // sorting
...
int compare (const void * a, const void * b){
    return ( (*(initInfo*)a).id - (*(initInfo*)b).id );
}
